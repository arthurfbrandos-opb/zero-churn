...
-- ─── TABELA: agencies ─────────────────────────────────────────
-- Uma agência = um tenant isolado por RLS
...
-- ─── TABELA: agency_users ─────────────────────────────────────
-- Liga usuários do Supabase Auth a uma agência
...
create table if not exists client_integrations (
  ...
  credentials_enc  text,   -- JSON criptografado com AES-256
...
create table if not exists form_submissions (
  ...
  ip_hash          text  -- LGPD: hash do IP, não o IP bruto
);
...
-- ─── FUNÇÃO HELPER: get_agency_id() ──────────────────────────
-- Retorna o agency_id do usuário logado (via Supabase Auth)
...
create policy "Ver própria agência"
  on agencies for select
  using (id = get_agency_id());

create policy "Editar própria agência"
  on agencies for update
  using (id = get_agency_id());

-- ─── RLS POLICIES: agency_users ───────────────────────────────
create policy "Ver membros da própria agência"
  on agency_users for select
  using (agency_id = get_agency_id());

create policy "Inserir via service_role"
  on agency_users for insert
  with check (true);  -- service_role ignora RLS, mas garante a policy

-- ─── RLS POLICIES: clients ────────────────────────────────────
create policy "CRUD de clientes da agência"
  on clients for all
  using (agency_id = get_agency_id())
  with check (agency_id = get_agency_id());

-- ─── RLS POLICIES: churn_records ──────────────────────────────
create policy "CRUD churn_records da agência"
  on churn_records for all
  using (agency_id = get_agency_id())
  with check (agency_id = get_agency_id());

-- ─── RLS POLICIES: client_integrations ───────────────────────
create policy "CRUD integrações da agência"
  on client_integrations for all
  using (agency_id = get_agency_id())
  with check (agency_id = get_agency_id());

-- ─── RLS POLICIES: health_scores ─────────────────────────────
create policy "CRUD health_scores da agência"
  on health_scores for all
  using (agency_id = get_agency_id())
  with check (agency_id = get_agency_id());

-- ─── RLS POLICIES: action_items ──────────────────────────────
create policy "CRUD action_items da agência"
  on action_items for all
  using (agency_id = get_agency_id())
  with check (agency_id = get_agency_id());

-- ─── RLS POLICIES: form_tokens ───────────────────────────────
create policy "CRUD form_tokens da agência"
  on form_tokens for all
  using (agency_id = get_agency_id())
  with check (agency_id = get_agency_id());

-- Formulário público: leitura via token sem autenticação
create policy "Formulário público lê token por valor"
  on form_tokens for select
  using (true);  -- filtro pelo token é feito na query, sem RLS

-- ─── RLS POLICIES: form_submissions ──────────────────────────
create policy "CRUD submissions da agência"
  on form_submissions for all
  using (agency_id = get_agency_id())
  with check (agency_id = get_agency_id());

-- Formulário público: inserção sem autenticação (via service_role na API)
-- (a inserção pública é feita via API route com service_role, não direto)

-- ─── RLS POLICIES: alerts ────────────────────────────────────
create policy "CRUD alertas da agência"
  on alerts for all
  using (agency_id = get_agency_id())
  with check (agency_id = get_agency_id());

-- ─── RLS POLICIES: analysis_logs ─────────────────────────────
create policy "CRUD logs da agência"
